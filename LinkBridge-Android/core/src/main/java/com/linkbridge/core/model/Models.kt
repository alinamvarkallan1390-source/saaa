...
data class LinkSnapshot(val state:LinkState=LinkState.IDLE,val device:DeviceTelemetry=DeviceTelemetry(),val message:String="",val retryInMs:Long=0) { val quality:String get()=when(device.rssi){in -59..0->"عالی";in -69..-60->"خوب";in -79..-70->"متوسط";else->"ضعیف"}; val distanceMeters:Double get()=Math.pow(10.0,(-59-device.rssi)/20.0) }
